...
const CONFIG = {
    VOLTAGE_CALIBRATION_OFFSET: 0.4,
    LIPO_CURVE: [
        { v: 12.6, p: 1.0 },
        { v: 12.2, p: 0.85 },
        { v: 12.0, p: 0.75 },
...
function calcAnimDuration(speed) {
    let abs = Math.abs(speed);
    if (abs < 20) return '0s';
    return (2.0 - (abs / 255) * 1.8).toFixed(2) + 's';
}

function applyCurve(val, multiplier) {
    let sign = val < 0 ? -1 : 1;
    let abs = Math.abs(val);
    return sign * (abs * abs * abs) * multiplier;
}
...
window.crawlerAPI.onTelemetry((data) => {
    if (data.battery !== undefined) {
        ...
        let rawVoltage = data.battery + CONFIG.VOLTAGE_CALIBRATION_OFFSET;
        
        // Exponential Moving Average (EMA) to smooth out voltage sag when driving motors
        if (batteryEma === null) {
            batteryEma = rawVoltage;
        } else {
            // Alpha of 0.1 provides smooth tracking without jumping instantly
            batteryEma = (rawVoltage * 0.1) + (batteryEma * 0.9);
        }
        
        let calibratedVoltage = batteryEma;
        
        let pct = 0;
        if (calibratedVoltage >= 12.6) pct = 1.0;
        else if (calibratedVoltage <= 10.8) pct = 0;
        else {
            for (let i = 0; i < CONFIG.LIPO_CURVE.length - 1; i++) {
                if (calibratedVoltage <= CONFIG.LIPO_CURVE[i].v && calibratedVoltage >= CONFIG.LIPO_CURVE[i+1].v) {
                    let v_range = CONFIG.LIPO_CURVE[i].v - CONFIG.LIPO_CURVE[i+1].v;
                    let p_range = CONFIG.LIPO_CURVE[i].p - CONFIG.LIPO_CURVE[i+1].p;
                    let v_ratio = (calibratedVoltage - CONFIG.LIPO_CURVE[i+1].v) / v_range;
                    pct = CONFIG.LIPO_CURVE[i+1].p + (p_range * v_ratio);
                    break;
                }
            }
        }
        ...
        if (badgeBattery) {
            const rgb = hexToRgb(batteryColor);
            badgeBattery.style.borderColor = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.5)`;
            badgeBattery.style.boxShadow = `0 0 12px rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.3)`;
...
function updateGamepad() {
    try {
        ...
        if (gp) {
            ...
            if (Math.abs(gp.axes[1]) > CONFIG.DEADZONE) {
                let normalized = -gp.axes[1];
                leftSpeed = Math.round(applyCurve(normalized, speedMultiplier) * 255 * CONFIG.LEFT_TRIM);
            }
            if (Math.abs(gp.axes[3]) > CONFIG.DEADZONE) {
                let normalized = -gp.axes[3];
                rightSpeed = Math.round(applyCurve(normalized, speedMultiplier) * 255 * CONFIG.RIGHT_TRIM);
            }
            ...
            if (crawlerSvg) {
                let lDur = calcAnimDuration(leftSpeed);
                let rDur = calcAnimDuration(rightSpeed);
        ...
        if (noSignal) {
            ...
            noSignal.style.color = 'red';
            noSignal.style.fontSize = '12px';
            noSignal.innerText = "JS CRASH: " + (e.stack || e);
        }
    }
