...
void setBaseInfoFeedbackMode(byte inputCmd) {
	if (inputCmd == 1) {
		baseFeedbackFlow = 1;
	} else if (inputCmd == 0) {
		baseFeedbackFlow = 0;
...
void baseInfoFeedback() {
	...
	jsonInfoHttp["M1"] = ddsm_fb_1;
  
	jsonInfoHttp["M2"] = ddsm_fb_2;
	jsonInfoHttp["M3"] = ddsm_fb_3;
	jsonInfoHttp["M4"] = ddsm_fb_4;
