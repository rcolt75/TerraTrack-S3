...
int DDSM_CTRL::ddsm115_fb(int * ddsm_speed_1,int * ddsm_speed_2,int * ddsm_speed_3,int * ddsm_speed_4,int * ddsm_posl ,int * ddsm_posr) {
int Size = 0;
  int ComData;
  uint8_t data[10];
  uint8_t crc = 0;
  ...
  if (crc == data[9]){
    ...
    switch(ddsm_id){
        case 1: * ddsm_speed_1=speed_data;
                * ddsm_posl = ddsm_pos;                 
        break;
        case 2: * ddsm_speed_2=-speed_data;
                * ddsm_posr = ddsm_pos;                
                // Serial.print("ddsm_speed_2 ");
                // Serial.println(*ddsm_speed_2);
                // Serial.print("*ddsm_posr ");
                // Serial.println(*ddsm_posr);   
        break;
        case 3: * ddsm_speed_3=-speed_data;
        break;
        case 4: * ddsm_speed_4=speed_data;
        break;
    } 
  } 
  return Size;
...
// feedback data from ddsm400.
int DDSM_CTRL::ddsm400_fb(int * ddsm_speed_1,int * ddsm_speed_2,int * ddsm_speed_3,int * ddsm_speed_4,int * id,int * ddsm_posl,int * ddsm_posr) {
  // unsigned long startTime = millis();
  int Size = 0;
  int ComData;
  uint8_t data[10];
  ...
  //if (pSerial->available() >= 10) {
    int feedback_type = data[1];
    uint8_t ID = data[0];
    *id =ID;
    if (feedback_type == 0x65){
   ...
   //Serial.println(*id);
    ...
    else if(feedback_type == 0x75)
    {
      int ddsm_pos = (data[6] << 8) | data[7];
       switch(ID){
          case 1: * ddsm_posl=ddsm_pos;
          break;
          case 2: * ddsm_posr=ddsm_pos;
          break;
      } 
...
void DDSM_CTRL::ddsm_get_pos(uint8_t id) {  
  packet_move[0] = id;
  packet_move[1] = 0x74;
...
void DDSM_CTRL::ddsm_change_id(uint8_t id) {
packet_move[0] = 0xAA;
...
void DDSM_CTRL::ddsm_get_info(uint8_t id) {  
  ...
  pSerial->write(packet_move, packet_length);
  if (ddsm_type == TYPE_DDSM115) {
  	//ddsm115_fb(int * ddsm_speed_1,int * ddsm_speed_2,int * ddsm_speed_3,int * ddsm_speed_4,int * ddsm_posl ,int * ddsm_posr);
  } else if (ddsm_type == TYPE_DDSM210) {
  	ddsm210_fb();
  }
}
...
void DDSM_CTRL::ddsm400_enable(uint8_t id) {
  packet_move[0] = id;
  packet_move[1] = 0xA0;
...
void DDSM_CTRL::ddsm400_speedLoop(uint8_t id) {
  packet_move[0] = id;
  packet_move[1] = 0xA0;
...
void DDSM_CTRL::ddsm400_enable_1() {
  packet_move[0] = 0x01;
  packet_move[1] = 0xA0;
...
void DDSM_CTRL::ddsm400_enable_2() {
